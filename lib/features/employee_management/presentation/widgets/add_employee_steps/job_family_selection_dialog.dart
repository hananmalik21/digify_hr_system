...
class _JobFamilySelectionDialogState extends ConsumerState<JobFamilySelectionDialog> {
  ...
  void initState() {
    ...
    _paginationListener = PaginationScrollListener(
      ...
      threshold: 500.0,
      onLoadMore: () {
        ref.read(employeeJobFamilyNotifierProvider(widget.enterpriseId).notifier).loadNextPage();
      },
    );
            ...
            children: [
              OrgUnitSelectionHeader(
                ...
                onSearchChanged: (value) {
                  if (value.isEmpty) {
                    ref.read(employeeJobFamilyNotifierProvider(widget.enterpriseId).notifier).clearSearch();
                  } else {
                    ref.read(employeeJobFamilyNotifierProvider(widget.enterpriseId).notifier).search(value);
                  }
                },
