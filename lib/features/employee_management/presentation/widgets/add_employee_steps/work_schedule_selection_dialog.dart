...
class EmployeeWorkScheduleSelectionDialog extends ConsumerStatefulWidget {
  ...
  static Future<WorkSchedule?> show({
    ...
    return await showDialog<WorkSchedule>(
      ...
      builder: (context) =>
          EmployeeWorkScheduleSelectionDialog(enterpriseId: enterpriseId, selectedScheduleId: selectedScheduleId),
    );
  }
...
class _EmployeeWorkScheduleSelectionDialogState extends ConsumerState<EmployeeWorkScheduleSelectionDialog> {
  ...
  void _scrollListener() {
    if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent * 0.8) {
      ref.read(employeeWorkSchedulesNotifierProvider(widget.enterpriseId).notifier).loadNextPage();
    }
  }
  ...
  Widget build(BuildContext context) {
    final schedulesState = ref.watch(employeeWorkSchedulesNotifierProvider(widget.enterpriseId));
      ...
      child: Column(
        ...
        children: [
          ...
          SearchField(
            ...
            onChanged: (value) {
              ref.read(employeeWorkSchedulesNotifierProvider(widget.enterpriseId).notifier).search(value);
            },
            onClear: () {
              ref.read(employeeWorkSchedulesNotifierProvider(widget.enterpriseId).notifier).search('');
            },
            initialValue: ref.read(employeeWorkSchedulesNotifierProvider(widget.enterpriseId)).searchQuery ?? '',
          ),
        ],
      ),
    );
  ...
  Widget _buildContent(
    ...
    if (error != null && items.isEmpty) {
      return OrgUnitSelectionErrorState(
        error: error,
        onRetry: () => ref.read(employeeWorkSchedulesNotifierProvider(widget.enterpriseId).notifier).refresh(),
      );
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return WorkScheduleListItem(schedule: schedule, isSelected: isSelected, onTap: () => context.pop(schedule));
      },
    );
  }
}
