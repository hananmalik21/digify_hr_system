    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          // Cancel button
          TextButton(
            onPressed: saveState.isSaving ? null : () => Navigator.of(context).pop(),
            style: TextButton.styleFrom(
              padding: EdgeInsets.symmetric(horizontal: isMobile ? 16.w : 24.w, vertical: isMobile ? 12.h : 14.h),
                ...
                : () async {
                    ...
                    if (currentEditState.selectedEnterpriseId == null) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Please select an enterprise'), backgroundColor: Colors.red),
                      );
                      return;
                    }
                    ...
                    if (currentEditState.structureName.trim().isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Structure name is required'), backgroundColor: Colors.red),
                      );
                      return;
                    }
                    ...
                    if (currentEditState.levels.isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('At least one level is required'), backgroundColor: Colors.red),
                      );
                      return;
                    }
                    ...
                    try {
                      ...
                      if (success) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Structure saved successfully'), backgroundColor: Colors.green),
                        );
                      }
